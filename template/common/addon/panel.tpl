...
$t 		   = DIR_TEMPLATE.$config->get('config_template');
 
$patterns =  $this->getPattern( $themeName );

if (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1'))) {
    $base =$config->get('config_ssl');
} else {
    $base =$config->get('config_url');
}


$backgroundImageURL = $base.'catalog/view/theme/'.$themeName.'/image/pattern/';
$directories = glob( $this->_themeDir . '/stylesheet/skins/*.css' );
...
<div class="paneltool themetool">
	...
	<div class="panelcontent ">
		<div class="panelinner">
			<h4>Panel Tool</h4>
			<form method="post" class="clearfix"><div class="clearfix">
				<p class="group-input">
					<label>Theme</label>
					<select name="userparams[skin]">
						<option value=""><?php echo $objlang->get('default');?></option>
				...
				<p class="group-input">
					...
					<a class="btn btn-default btn-outline-small" href="<?php echo HTTP_SERVER; ?>?pavreset=?"><span>Reset</span></a>
				</p>
			</div></form>
		</div>	
	</div>
...
<div class="paneltool editortool">
	...
	<div class="panelcontent editortool"><div class="panelinner">
		<?php if( class_exists("ThemeControlHelper") ){ ?>				
			...
			<?php $xmlselectors = ThemeControlHelper::renderEdtiorThemeForm($config->get('config_template') ); ?>
			                              	...
			                              	<?php foreach ($group['selector'] as $item ) {  ?>
												 <?php  if (isset($item['type'])&&$item['type']=="image") { ?>
												  <div class="form-group background-images"> 
														...
														<a class="clear-bg btn btn-danger" href="#">Clear</a>
				                                  ...
				                                  <?php } elseif( $item['type'] == "fontsize" ) { ?>
				                                   <div class="form-group">
													...
													<option value="<?php echo $fs; ?>"><?php echo $fs; ?></option>
															...
															<a href="#" class="clear-bg btn btn-danger">Clear</a>
				                                  </div>
				                                  <?php } else { ?>
				                                  <div class="form-group">
														<label><?php echo $item['label']?></label>
														<input value="" size="10" name="customize[<?php echo $group['match']?>][]" data-match="<?php echo $group['match']?>" type="text" class="input-setting" data-selector="<?php echo $item['selector']?>" data-attrs="<?php echo $item['attrs']?>"><a href="#" class="clear-bg btn btn-danger">Clear</a>
												  </div>
				                                  <?php } ?>
